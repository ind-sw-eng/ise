...
- **Schema.org markup**: One `Course` entity on the home page, `CreativeWork` on each project page, `BreadcrumbList` site-wide (see `_includes/layout/head.html` and `_includes/breadcrumb.html`).
- **Open Graph / Twitter tags**: Social sharing preview, using `/assets/images/hero.jpg` as the share image.
- **Sitemap**: Generated by `jekyll-sitemap` at `/sitemap.xml`. Do not commit a static `sitemap.xml` - it overrides the generated one and silently freezes the list of URLs.
- **Excluding a page**: Set `noindex: true` and `sitemap: false` in its front matter.
